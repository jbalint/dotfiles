;;; gud-yt.el --- GUD Mode for YellowTree debugger
;; Write me some more docs
;; You must (require 'gud) for this to work

(defcustom gud-yt-command-name
  "java -agentpath:/home/jbalint/Desktop/Work/misc/YellowTree/libyt.so -cp . Test"
...
(defun encode-java-type (type)
  (let* ((native-type-code (cdr (assoc type java-primitive-types)))
	...
	(or native-type-code
		;; (naive?) assume it's a full name if it has a pkg (/)
		(concat (if (string-match "/" internal-name) "L" "M") internal-name ";"))))
;;(encode-java-type "Dint")
...
(defun yt (command-line)
  ...
  ;;  (gud-def gud-cont "cont" "\C-r" "Continue program")
