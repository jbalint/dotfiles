...
- **Mirrors convention:** When creating an MDB project issue that mirrors an upstream MongoDB Jira ticket (e.g. SERVER-*, DEVPROD-*, PERF-*, BF-*, PMBT-*), include the upstream key in the summary (e.g. `SERVER-133645 JOO/CBR cardinality snapshot mismatch...`) and set the Mirrors custom field (`customfield_18500`) to the full upstream Jira URL. Use `jira issue edit <KEY> --custom Mirrors="<URL>" --no-input` (the CLI emits a warning about unconfigured fields but the update applies correctly).
